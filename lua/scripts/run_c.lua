...
-- Function to find and compile a C file with int main()
M.run_c_file = function()
    -- Get the directory of the current file
    local current_dir = vim.fn.expand("%:p:h")

    -- Find a C file in the current directory with "int main()"
    ...
    for _, file in ipairs(vim.fn.readdir(current_dir)) do
        if file:match("%.c$") then
            ...
            local has_main = vim.fn.systemlist("grep -q 'int main' " .. file_path .. " && echo 'yes' or echo 'no'")[1]
    ...
    -- If no C file with int main() is found, print an error and return
    if not c_file_with_main then
        print("Error: No C file with 'int main()' found in the current directory.")
        return
    end
